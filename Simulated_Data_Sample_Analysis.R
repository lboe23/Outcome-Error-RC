...
WHI_data<-read.csv("U://Paper 1//Final Code//Simulated_Data_Example.csv")
...
WHI_data_wide <- dcast(WHI_data, log_FFQ_energy_c+bmi_c+age_c+black+hispanic+other+income2+income3+income5+income6+texpwk+educ3+educ5+Biom_Energy_m+
                         htn+alcohol1_nondrinker+alcohol3_moderateheavy+ID ~ testtime, value.var="true_result")


correction_energy<-lm(Biom_Energy_m~log_FFQ_energy_c+bmi_c+age_c+black+hispanic+other+income2+income3+income5+income6+texpwk+educ3+educ5+htn+alcohol1_nondrinker+alcohol3_moderateheavy,data=WHI_data_wide)
...
keep_naive<-unlist(tapply(WHI_data$result,WHI_data$comid,after_first_pos))
datafinal_naive<-WHI_data[keep_naive,]
...
datas1 <- WHI_data[which(WHI_data$os_dm_age=="OS_age50s"),]
datas2 <- WHI_data[which(WHI_data$os_dm_age=="OS_age60s"),]
datas3 <- WHI_data[which(WHI_data$os_dm_age=="OS_age70s"),]
datas4 <- WHI_data[which(WHI_data$os_dm_age=="DM_age50s"),]
datas5 <- WHI_data[which(WHI_data$os_dm_age=="DM_age60s"),]
datas6 <- WHI_data[which(WHI_data$os_dm_age=="DM_age70s"),]

subject1<-datas1$comid
testtime1<-datas1$testtime
result1<-datas1$result

subject2<-datas2$comid
testtime2<-datas2$testtime
result2<-datas2$result

subject3<-datas3$comid
testtime3<-datas3$testtime
result3<-datas3$result

subject4<-datas4$comid
testtime4<-datas4$testtime
result4<-datas4$result

subject5<-datas5$comid
testtime5<-datas5$testtime
result5<-datas5$result

subject6<-datas6$comid
